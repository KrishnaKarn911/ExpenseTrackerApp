...
form.addEventListener('submit', async (e) => {
    ...
    try {
        ...
        const response = await axios.post('http://3.110.159.176:3000/exp/expense', {
...
async function fetchExpenses(page = 1, limit = 2) {
    try {
        ...
        const response = await axios.get(`http://3.110.159.176:3000/exp/expense?page=${page}&limit=${limit}`, { headers: { "Authorization": token } });
...
async function deleteExpense(expenseId) {
    try {
        ...
        await axios.delete(`http://3.110.159.176:3000/exp/expense/${expenseId}`, { headers: { "Authorization": token } });
    } catch (err) {
...
document.getElementById('buy-premium-btn').addEventListener('click', async () => {
    try {
        ...
        const response = await axios.get('http://3.110.159.176:3000/purchase/premiummembership', {
            headers: { "Authorization": token }
        });
        ...
        const options = {
            ...
            "handler": async function (response) {
                await axios.post('http://3.110.159.176:3000/purchase/updateTransactionStatus', {
...
async function showleaderboard() {
    try {
        const token = localStorage.getItem('token');
        const response = await axios.get('http://3.110.159.176:3000/premium/showleaderboard', { headers: { "Authorization": token } });
...
async function fetchReports() {
    try {
        ...
        const response = await axios.get('http://3.110.159.176:3000/premium/reports', {
            headers: { "Authorization": token }
        });
       ...
       localStorage.setItem('reportsData', JSON.stringify(response.data.result))
        if (response.data.status === 'success') {
            ...
            window.location.href = 'http://3.110.159.176:3000/premium/reportsPage'
        } else {
